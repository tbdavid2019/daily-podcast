import { keepDays, podcastDescription, podcastOwner, podcastTitle } from '@/config'
...
export function buildLlmsTxt(baseUrl: string) {
  return [
    ...
    '',
    `- [${podcastTitle}首頁](${baseUrl}/): 展示最新 ${keepDays} 天每日科技新聞摘要與 Podcast 音訊播放器。`,
    `- [節目文章頁面](${baseUrl}/post/{date}): 包含指定日期（如 ${baseUrl}/post/2026-08-10）的完整報導內文、故事導讀與延伸參考連結。`,
    `- [Podcast RSS Feed](${baseUrl}/rss.xml): 提供適用於各大 Podcast 播放器的標準 RSS 訂閱源。`,
    `- [Sitemap](${baseUrl}/sitemap.xml): 完整網站頁面索引與搜尋檢索地圖。`,
    '',
...
export function buildLlmsFullTxt(baseUrl: string) {
  return [
    ...
    '',
    `### 1. 首頁 (\`${baseUrl}/\`)`,
    `- **功能**: 展示近 ${keepDays} 天的 Podcast 集數列表與線上音訊播放器。`,
    ...
    '- **功能**: 提供符合 PodcastIndex 規範的 RSS XML，包含音訊檔 enclosure 與文章連結。',
    '- **快取策略**: 邊緣快取 10 分鐘，支援跨網域 CORS GET 請求。',
    '',
    `### 4. 網站地圖 (\`${baseUrl}/sitemap.xml\`)`,
    `- **功能**: 輸出最近 ${keepDays} 天內所有文章的 XML 地圖供 AI 爬蟲與檢索機器人索引。`,
    '',
