...
| `1.0` | 正常速度 | 標準播放 | ✅ |
| `1.1` | 快 10% | 稍快，自然 | ✅ |
| `1.3` | 快 30% | **預設值**，效率與清晰度平衡 | ⭐ 推薦 |
| `1.5` | 快 50% | 快速瀏覽 | ✅ |
| `2.0` | 快 100% | 極速，可能影響理解 | ⚠️ |

#### 不同 TTS 提供者的支援

| TTS Provider | 支援 AUDIO_SPEED | 預設值 | 備註 |
|--------------|------------------|--------|------|
| **Edge TTS** | ✅ | `10%` | 使用百分比格式（如 `10%`、`20%`） |
| **OpenAI TTS** | ✅ | `1.3` | 使用倍數格式（範圍 0.25-4.0） |
| **Minimax TTS** | ✅ | `1.1` | 使用倍數格式 |
...
調整語速可以讓你在相同播放時長內容納更多內容：

| 原始時長 | 1.0x | 1.3x（預設） | 1.5x | 節省時間 |
|----------|------|--------------|------|----------|
| 30 分鐘 | 30 分鐘 | **23 分鐘** | 20 分鐘 | 7-10 分鐘 |
| 45 分鐘 | 45 分鐘 | **35 分鐘** | 30 分鐘 | 10-15 分鐘 |
| 60 分鐘 | 60 分鐘 | **46 分鐘** | 40 分鐘 | 14-20 分鐘 |
