...
export function ArticleCard({ article, staticHost = '', showSummary = false, showFooter = false }: ArticleCardProps) {
  ...
  const stickyHeader = (
    <div className={`sticky top-0 z-30 bg-white/90 backdrop-blur-md ${showFooter ? 'border-b border-zinc-200/30 rounded-t-lg' : 'rounded-lg'}`}>
        ...
        ? (
            <Tabs defaultValue="summary" className="w-full">
              ...
              <CardFooter className="flex-col pt-4 bg-zinc-50/20 rounded-b-lg">
                <TabsContent value="summary" className="w-full prose prose-zinc max-w-none py-6 prose-a:no-underline hover:prose-a:underline prose-p:leading-relaxed">
                  {article.blogContent && (
                    <div dangerouslySetInnerHTML={{ __html: markdownRenderer.render(article.blogContent) }} />
                  )}
                </TabsContent>
                <TabsContent value="podcast" className="w-full prose prose-zinc max-w-none whitespace-pre-line py-6 leading-relaxed text-zinc-700">
