  ...
  async run(event: WorkflowEvent<Params>, step: WorkflowStep) {
    ...
    try {
      ...
      if (existingContent) {
        ...
        // Clear any stale lock since content exists
        await this.env.HACKER_NEWS_KV.delete(lockKey)
      ...
      // Clear the workflow lock after successful completion
      await step.do('clear workflow lock', retryConfig, async () => {
        await this.env.HACKER_NEWS_KV.delete(lockKey)
        console.info('Workflow lock cleared for:', today)
        return 'lock cleared'
      })
      ...
      // Clear the workflow lock on error to prevent permanent lockout
      try {
        await this.env.HACKER_NEWS_KV.delete(lockKey)
        console.info('Workflow lock cleared due to error for:', today)
      }
      catch (cleanupError) {
        console.error('Failed to clear workflow lock:', cleanupError)
      }
      throw error
    }
  }
